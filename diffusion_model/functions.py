...
from sklearn.metrics import (
    ...
    classification_report, precision_recall_curve, precision_recall_fscore_support, ConfusionMatrixDisplay, average_precision_score
)
...
def compute_delta_r(hits):
    """
    Computes Δr between hit layers (1A vs 1B).
...
def analytic_delta_z(hits, bx, by, bz):
    """
    Optimistic model: Δz = Δr * |cot(theta)|
    Uses barycenter direction for θ.
    """
    Δr = compute_delta_r(hits)
    if Δr is None:
        return None
