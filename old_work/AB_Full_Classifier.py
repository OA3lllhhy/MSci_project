...
parser.add_argument('--test', action ='store_true', help='Test Neural Network with different random seed')
...
if args.run:
    all_configs = {
        'muons': {
            ...
            'outfile': '/ceph/submit/data/user/h/haoyun22/process_data_FCC_background/ABmuons_edep_xB_label_v3.1.pkl'
        },
        'signal': {
            'files': glob.glob('/ceph/submit/data/group/fcc/ee/detector/VTXStudiesFullSim/CLD_wz3p6_ee_qq_ecm91p2/*.root'),
            'outfile': '/ceph/submit/data/user/h/haoyun22/process_data_FCC_background/ABsignal_edep_xB_label_v3.1.pkl'
        },
        'background': {
            'files': glob.glob('/ceph/submit/data/group/fcc/ee/detector/VTXStudiesFullSim/CLD_o2_v05/FCCee_Z_4IP_04may23_FCCee_Z/*.root'),
            'outfile': '/ceph/submit/data/user/h/haoyun22/process_data_FCC_background/ABbkg_edep_xB_label_v3.1.pkl'
        }
    }
        ...
        for i, filename in enumerate(files):
            ...
            for event in events:
                ...
                for trackID, hit_group in particle_hits.items():
                    ...
                    cross_B = int(any(functions.in_1B(hit) for hit in p.hits))
...
if args.classify:
    ...
    plt.legend(loc='best')
    plt.grid(True)
    # plt.savefig(os.path.join(outdir, "threshold_sweep_metrics_xB_label_v3_0.99.png"))
    plt.close()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(outdir, "ROC_curve_xB_label_v3.png"))
    plt.close()
    ...
    plt.savefig(os.path.join(outdir, "confusion_matrix_xB_label_v3.png"))
    plt.close()
...
#     ],
#     outdir="Classification_AB",
#     filename="feature_importance_xB_label_data_v3.png",
#     sort=True
# )
...
if args.neural:
    ...
    # === Load data ===
    with open('/ceph/submit/data/user/h/haoyun22/process_data_FCC_background/ABmuons_edep_xB_label_v3.1.pkl', 'rb') as f: # ABmuons_edep_xB_label.pkl
        muons = pickle.load(f)
    with open('/ceph/submit/data/user/h/haoyun22/process_data_FCC_background/ABsignal_edep_xB_label_v3.1.pkl', 'rb') as f: # ABsignal_edep_xB_label.pkl
        signal = pickle.load(f)
    with open('/ceph/submit/data/user/h/haoyun22/process_data_FCC_background/ABbkg_edep_xB_label_v3.1.pkl', 'rb') as f: # ABbkg_edep_xB_label.pkl
        background = pickle.load(f)
...
if args.test:
    """
    Test Neural Network classifier with 3000 random seeds, training multiple times and store AUC scores for each run,
    and calculate mean and std deviation of AUC scores across runs.
    1. Set random seeds
    2. Load data
    3. Prepare DataLoaders
    4. Define model, loss, optimizer
    5. Train and evaluate model for each seed
    6. Store and print results
    7. Calculate mean and std deviation of AUC scores
    ...
    from torch.utils.data import DataLoader, TensorDataset
    from sklearn.model_selection import train_test_split
    from functions import relabel_noise_clusters, get_features_and_labels, train_neural_network, evaluate_model
    from functions import ParticleClassifier
    import numpy as np
    import random
    import pickle
    ...
    import pandas as pd
    from functions import evaluate_model_repeat
    ...
    # === Load data ===
    with open('/ceph/submit/data/user/h/haoyun22/process_data_FCC_background/ABmuons_edep_xB_label_v3.1.pkl', 'rb') as f:
        muons = pickle.load(f)
    with open('/ceph/submit/data/user/h/haoyun22/process_data_FCC_background/ABsignal_edep_xB_label_v3.1.pkl', 'rb') as f:
        signal = pickle.load(f)
    with open('/ceph/submit/data/user/h/haoyun22/process_data_FCC_background/ABbkg_edep_xB_label_v3.1.pkl', 'rb') as f:
        background = pickle.load(f)
    ...
    print(f"Starting runs with different random seeds...")
    for i in range(n_repeat):
        ...
        criterion = nn.BCELoss()
        optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
        save_path = f"{checkpoint_path}/Train_checkpoint.pth"
        ...
        results = functions.evaluate_model_repeat(
            checkpoint_path=save_path,
            model_class=ParticleClassifier,
            input_dim=input_dim,
            test_loader=test_loader,
            save_dir=None,
            threshold_plot_dir=None,
            cm_path=None
        )
